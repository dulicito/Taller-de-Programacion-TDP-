...
código del producto (entre 1 y 15) y cantidad vendida (como máximo 99 unidades). El código y el dia deben generarse 
automáticamente (random) y la cantidad se debe leer. El ingreso de las ventas finaliza con el día de venta 0 
(no se procesa).
...
e. Un módulo que elimine, del vector ordenado, las ventas con código de producto entre dos valores que se ingresan 
como parámetros. 

f. Un módulo que muestre el contenido del vector resultante del punto e).

g. Un módulo que retorne la información (ordenada por código de producto de menor a mayor) de cada código par de 
producto junto a la cantidad total de productos vendidos.
...
type dias = 0..31;
	 ...
	 vector = array [1..dimF] of venta; // inciso A 
	 elemento = record
	              codigoP: rango1;
	              cantTotal: integer;
	            end;
...
begin
  while (L <> nil) do begin
    writeln('Codigo: ', L^.dato.codigoP, ' | Total vendido: ', L^.dato.cantTotal);
    L := L^.sig;
  end;
end;
...
Begin
  ...
  if (dimL = 0) then writeln ('--- Vector sin elementos ---')
                       ...
                       Eliminar (v, dimL, valorInferior, valorSuperior); //inciso E
